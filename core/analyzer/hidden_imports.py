...
class HiddenImportsManager:
    ...
    def get_hidden_imports(
        ...
        # ========== 第二层：第三方库自动分析 ==========
        hidden.extend(self._get_third_party_hidden_imports(dependencies, hidden, is_real_package_func, is_stdlib_func))

        # 去重
        hidden = list(set(hidden))

        # 验证：过滤掉当前环境中不存在的模块（避免 Nuitka FATAL error）
        hidden = self._filter_existing_modules(hidden)

        return hidden

    @staticmethod
    def _module_exists(module_name: str) -> bool:
        """检查模块是否存在于当前 Python 环境中"""
        try:
            return importlib.util.find_spec(module_name) is not None
        except (ValueError, ImportError, ModuleNotFoundError):
            return False

    def _filter_existing_modules(self, hidden_imports: List[str]) -> List[str]:
        """过滤掉当前环境中不存在的隐藏导入模块"""
        filtered = []
        removed = []
        ...
        if removed:
            self.log(f"  已过滤 {len(removed)} 个不存在的隐藏导入模块: {', '.join(sorted(removed))}")
        return filtered

    def _get_qt_hidden_imports(self, dependencies: Set[str], primary_qt: Optional[str]) -> List[str]:
        """获取 Qt 框架的隐藏导入"""
        hidden = []
    ...
    def _get_data_science_hidden_imports(self, dependencies: Set[str]) -> List[str]:
        ...
        # numpy 相关
        # 注意：numpy 1.24+ 重构了内部模块结构，部分旧版子模块已移除
        if "numpy" in dependencies:
            hidden.extend(
                [
                    ...
                    "numpy.fft",
                    "numpy.polynomial",
                    # 以下模块在 numpy 1.24+ 中可能不存在，将在 validate 阶段过滤
                    "numpy.random.common",
    ...
    def _get_third_party_hidden_imports(
        ...
        for dep in dependencies:
            ...
            is_cached = (
                ...
                or dep_normalized in CONFIGURED_LIBRARIES
            ...
            # 检查是否已经在hidden中有相关导入
            has_hidden = (
                any(dep_lower in h.lower() for h in hidden)
                or any(module_lower in h.lower() for h in hidden)
                or any(h.lower().startswith(module_lower + ".") for h in hidden)
                or any(h.lower() == module_lower for h in hidden)
            )
